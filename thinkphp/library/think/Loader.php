...
class Loader
{
    ...
    public static function register($autoload = '')
    {
        ...
        // var_dump(self::$composerPath);exit;
        // Composer自动加载支持
        if (is_dir(self::$composerPath)) {
            if (is_file(self::$composerPath . 'autoload_static.php')) {
                ...
                // var_dump($declaredClass);exit;
                //取数组中最后一个对象（类名）
                $composerClass = array_pop($declaredClass);
                // var_dump($composerClass);
                foreach (['prefixLengthsPsr4', 'prefixDirsPsr4', 'fallbackDirsPsr4', 'prefixesPsr0', 'fallbackDirsPsr0', 'classMap', 'files'] as $attr) {
                    //判断对象的属性或方法是否存在，
                    if (property_exists($composerClass, $attr)) {
                        self::${$attr} = $composerClass::${$attr};
                        // var_dump($attr,self::${$attr});exit;
                    }
                }
            } else {
        ...
        // var_dump( self::$prefixDirsPsr4);exit;
        ...
        // var_dump(self::$fallbackDirsPsr4);exit;
    }
    ...
    public static function autoload($class)
    {
        ...
        if (isset(self::$classAlias[$class])) {
            //设置类的别名 
            // var_dump(self::$classAlias[$class]);
    ...
    private static function findFile($class)
    {
        if (!empty(self::$classMap[$class])) {
            // 类库映射
            // var_dump(123,self::$classMap[$class]);exit;
        ...
        //think\Error.php
        // var_dump($logicalPathPsr4);exit;
        $first = $class[0]; // => t
        // var_dump($class."####");
        // var_dump(self::$prefixLengthsPsr4);
        //在self::$prefixLengthsPsr4数组中是否有$first这个KEY;
        if (isset(self::$prefixLengthsPsr4[$first])) {
            //循环self::$prefixLengthsPsr4[$first]
            foreach (self::$prefixLengthsPsr4[$first] as $prefix => $length) {
                //判断key值在$class字串中第一次出现的位置下标
                if (0 === strpos($class, $prefix)) {
                    // var_dump($prefix."111");
                    //循环数组 prefixDirsPsr4[$prefix]
                    foreach (self::$prefixDirsPsr4[$prefix] as $dir) {
                        // var_dump(self::$prefixDirsPsr4,self::$prefixDirsPsr4[$prefix]);exit;
                        // var_dump($logicalPathPsr4,$length,substr($logicalPathPsr4, $length));exit;
                        //判断截取拼接后的字串是否是存在的文件
    ...
    private static function addPsr4($prefix, $paths, $prepend = false)
    {
        if (!$prefix) {
            ...
            // var_dump(123,self::$prefixDirsPsr4);exit;
        } elseif ($prepend) {
        ...
        // var_dump(self::$prefixLengthsPsr4,self::$fallbackDirsPsr4,123,self::$prefixDirsPsr4);exit;
    }
